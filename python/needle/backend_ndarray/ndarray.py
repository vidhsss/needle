...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if self.size!=prod(new_shape):
            ...
            return NDArray.make(new_shape,NDArray.compact_strides(new_shape), device=self.device, handle=self._handle)
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        # return  NDArray.make(new_shape,tuple(stride), device=self.device, handle=self._handle)
    ...
    def __getitem__(self, idxs):
        ...
        off = [stride * idx.start for stride, idx in zip(self.strides, idxs)]
        offset=0
        for o in off:
            offset+=o
        return NDArray.make(
    ...
    #         new_stride[i] *= -1
    #     # offset = sum([(self.shape[i] -1) * self.strides[i] for i in axes])
        
    #     print("passing")
    #     flipped= NDArray.make(self.shape, strides=new_stride, device=self._device, handle=self._handle, offset=new_offset)
    #     print("passing")
    ...
    def flip(self, axes):
        ...
        # flipped_array = NDArray.make(self.shape, strides=new_strides, offset=new_offset, device=self.device,handle=self._handle)
        # # add compact
        # return flipped_array
        axes = tuple(ax % self.ndim for ax in axes)

        # Copy the current strides and offset
        new_strides = list(self.strides)
        new_offset = self._offset  # Use self._offset if offset is stored as a private attribute

        # Adjust strides and offset to flip the array
        for ax in axes:
            new_offset += (self.shape[ax] - 1) * self.strides[ax]
            new_strides[ax] = -new_strides[ax]

        # Create a new NDArray with the updated strides and offset
        flipped_array = NDArray.make(
            shape=self.shape,
            strides=tuple(new_strides),
            device=self.device,
            handle=self._handle,  # Assuming handle is accessible
            offset=new_offset
        )

        # Immediately compact the array to rearrange data and ensure positive strides
        flipped_array = flipped_array.compact()
    ...
    def pad(self, axes):
        ...
        # new_array= NDArray.make(new_shape, device=self._device)
        ...
        # new_array[insert_slices] = self  # Ensure your ndarray supports this operation
        ...
        res=self.device.full(new_shape,0)
        insert_slices = tuple(
            slice(axes[i][0], axes[i][0] + self.shape[i])
            for i in range(self.ndim)
        )
        
        res[insert_slices] = self
        return res             
        ### END YOUR SOLUTION
