...
class Tensor(Value):
    ...
    def backward(self, out_grad=None):
        ...
        # print("backwards self",self.device)
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for n in reverse_topo_order:
        n.grad = sum_node_list(node_to_output_grads_list[n])
        if n.is_leaf():
            continue
        for i, grad in enumerate(n.op.gradient_as_tuple(n.grad, n)):
            k =  n.inputs[i]
...
def topo_sort_dfs(node, visited, topo):
    """Post-order DFS"""
    ### BEGIN YOUR SOLUTION
    visited.add(node)  
