...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        qk_t = self.matmul(q, k)/math.sqrt(q_dim)
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        q_ = self.q_projection(self.prenorm_q(q.reshape((batch_size * queries_len, q_dim))))
        k_ = self.k_projection(self.prenorm_k(k.reshape((batch_size * keys_values_len, k_dim))))
        v_ = self.v_projection(self.prenorm_v(v.reshape((batch_size * keys_values_len, v_dim))))


        q_ = q_.reshape((batch_size, queries_len, self.num_head, self.dim_head)).transpose(axes=(1, 2))
        k_ = k_.reshape((batch_size, keys_values_len, self.num_head, self.dim_head)).transpose(axes=(1, 2))
        v_ = v_.reshape((batch_size, keys_values_len, self.num_head, self.dim_head)).transpose(axes=(1, 2)) 

        out, probs = self.attn(q_, k_, v_) 
...
class TransformerLayer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.l1 = Linear(q_features, hidden_size, device=device, dtype=dtype)
        self.l2 = Linear(hidden_size, q_features, device=device, dtype=dtype)
        self.dropout = Dropout(dropout)
        self.relu = ReLU()
        ### END YOUR SOLUTION
    ...
    def forward(
        ...
        y = self.dropout(self.l2(self.dropout(self.relu(self.l1(self.layer_norm(x.reshape((batch_size * seq_len, x_dim))))))))
...
class Transformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        for _ in range(num_layers):
            layers.append(TransformerLayer( embedding_size, num_head=num_head, dim_head=dim_head,  hidden_size=hidden_size,dropout=dropout,causal=causal,device=device, dtype=dtype,) )
    ...
    def forward(
        ...
        positions = Tensor(ndarray.array(positions).broadcast_to((seq_len, batch_size)), device=self.device, dtype=self.dtype)

        pos_embedding = self.position_embedding(positions).transpose((0,1))
